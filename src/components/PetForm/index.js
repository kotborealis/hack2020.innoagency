...
import {Header, Checkbox, Dropdown, Form, Icon, Button} from 'semantic-ui-react';
import {
    petBreedsOptions,
    petColorsOptions,
    petEarsOptions,
...
export const PetForm = () => {
    ...
    return (
        <Form>
            ...
            <Form.Input fluid label='Карточка учета животного, №' id='common_info.card_number' placeholder='0000a-00'/>
            <Form.Select
                onChange={(e, {value}) =>
                    setPetKind(value)
                }
            ...
            <Form.Select fluid label='Пол' id='common_info.animal.gender' options={petGenders} placeholder={'Пол'}/>
            <label>Порода</label>
            <Dropdown placeholder={'Порода'} id="common_info.animal.breed" fluid search selection options={petBreeds}/>
            <label>Окрас</label>
            <Dropdown placeholder={'Окрас'} id="common_info.animal.color" fluid search selection options={petColors}/>
            <label>Шерсть</label>
            <Dropdown placeholder={'Шерсть'} id="common_info.animal.fur" fluid search selection options={petFur}/>
            <label>Уши</label>
            <Dropdown placeholder={'Уши'} id="common_info.animal.ears" fluid search selection options={petEars}/>
            <label>Хвост</label>
            <Dropdown placeholder={'Хвост'} id="common_info.animal.tail" fluid search selection options={petTail}/>
            <label>Размер</label>
            <Dropdown placeholder={'Размер'} id="common_info.animal.size" fluid search selection options={petSize}/>
            <Form.Input fluid label='Особые приметы' id='common_info.description' placeholder={'Особые приметы'}/>
            ...
            <Form.Input fluid label='Заказ-наряд/акт о поступлении животного, дата' id='catching_info.admission_date'
            ...
            {treatments.map((treatment, i) => <>
                <Form.Input fluid label='Дата'
                            onChange={(e, {value}) =>
                                setTreatments([...treatments.slice(0, i), {...treatment, date: value}, ...treatments.slice(i + 1)])}
                            value={treatment.date}
                            placeholder={'Дата'}/>
                <Form.Input fluid label='Название препарата'
                            onChange={(e, {value}) =>
                                setTreatments([...treatments.slice(0, i), {...treatment, drug_name: value}, ...treatments.slice(i + 1)])}
                            value={treatment.drug_name}
                            placeholder={'Название препарата'}/>
                <Form.Input fluid label='Доза'
                            onChange={(e, {value}) =>
                                setTreatments([...treatments.slice(0, i), {...treatment, dose: value}, ...treatments.slice(i + 1)])}
            ...
            {vaccinations.map((vaccination, i) => <>
                <Form.Input fluid label='Дата'
                            onChange={(e, {value}) =>
                                setTreatments([...treatments.slice(0, i), {...vaccination, date: value}, ...vaccination.slice(i + 1)])}
                            value={vaccination.date}
                            placeholder={'Дата'}/>
                <Form.Input fluid label='Вид вакцины'
                            onChange={(e, {value}) =>
                                setTreatments([...treatments.slice(0, i), {...vaccination, name: value}, ...vaccination.slice(i + 1)])}
                            value={vaccination.name}
                            placeholder={'Вид вакцины'}/>
                <Form.Input fluid label='№ серии'
                            onChange={(e, {value}) =>
                                setTreatments([...treatments.slice(0, i), {...vaccination, series: value}, ...vaccination.slice(i + 1)])}
